...
void CPlaceName::InjectHooks() {
    ...
    ReversibleHooks::Install("CPlaceName", "Remove", 0x571D80, &CPlaceName::Remove);
...
void CPlaceName::Init() {
    ...
    this->m_nAdditionalTimer = 250;
    */
}

// 0x571D80
void CPlaceName::Remove() {
    m_pZone = nullptr;
    m_nAdditionalTimer = 0;
}

// 0x571E60
void CPlaceName::Update() const {
    if (m_pZone) {
...
void CPlaceName::Process() {
    ...
    if ((smallestZone == m_pZone || CGame::currArea) && m_pZone ||
            smallestZone &&
            m_pZone &&
