...
void COnscreenCounterEntry::ProcessForDisplayCounter(eOnscreenCounter type) {
    int left = CTheScripts::ScriptSpace[m_nVarId] < 0 ? 0 : CTheScripts::ScriptSpace[m_nVarId];
...
// 0x44CB00
void COnscreenCounterEntry::SetColourID(eHudColours colourId) {
    m_nColourId = colourId;
}
