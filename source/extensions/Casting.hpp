//===- Casting.hpp - Allow flexible, checked, casts --*- C++ -*-===//
//
...
// From `llvm/Support/Casting.h` (With some minor changes)
namespace notsa {
//===----------------------------------------------------------------------===//
// simplify_type
//===----------------------------------------------------------------------===//
...
// 
// ==NOTSA==
// Unlike LLVM we also forward the `To` (Self) type to `classof`, this way
// we can minimize code changes,
// For example `CTask` stores the self type in `Type`, but for accessing 
template<typename To, typename From>
struct isa_impl {
    static inline bool doit(const From& Val) {
        if constexpr (std::is_base_of_v<To, From>) { // Always allow upcasts, and perform no dynamic check for them.
            return true;
        } else {
            return To::template classof<From, To>(&Val);
        }
    }
};
...
struct isa_impl_wrap {
    ...
    static bool doit(const From& Val) {
        return isa_impl_wrap<To, SimpleFrom, typename simplify_type<SimpleFrom>::SimpleType>::
...
struct cast_convert_val {
    ...
    static typename cast_retty<To, From>::ret_type doit(const From& Val) {
        return cast_convert_val<To, SimpleFrom, typename simplify_type<SimpleFrom>::SimpleType>::
...
struct cast_convert_val<To, FromTy, FromTy> {
    ...
    static typename cast_retty<To, FromTy>::ret_type doit(const FromTy& Val) {
        return *(std::remove_reference_t<typename cast_retty<To, FromTy>::ret_type>*)&const_cast<FromTy&>(Val);
    }
};
...
struct CastIsPossible<To, std::optional<From>> {
    static inline bool isPossible(const std::optional<From>& f) {
        ...
        return CastIsPossible<To, From>(f); // NB: Using `CastIsPossible` here instead of the original code
    }
};
...
struct UniquePtrCast : public CastIsPossible<To, From*> {
    ...
    using CastResultType = std::unique_ptr<
        std::remove_reference_t<typename detail::cast_retty<To, From>::ret_type>>;
...
struct CastInfo : public CastIsPossible<To, From> {
    ...
    using CastReturnType = typename detail::cast_retty<To, From>::ret_type;

    static inline CastReturnType doCast(const From& f) {
        return detail::cast_convert_val<
            To,
            From,
...
template<typename To, typename From>
struct CastInfo<To, From, std::enable_if_t<!detail::is_simple_type<From>::value>> {
